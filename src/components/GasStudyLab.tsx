...
export function GasStudyLab(){
  const [mode,setMode]=useState<Mode>("fill");const [index,setIndex]=useState(0);
  ...
  return <div className="gas-lab">
    <div className="gas-mode-tabs">{([['fill','穴埋め'],['classify','ドラッグ分類']] as Array<[Mode,string]>).map(([value,label])=><button className={mode===value?"active":""} onClick={()=>setMode(value)} key={value}>{label}</button>)}</div>
