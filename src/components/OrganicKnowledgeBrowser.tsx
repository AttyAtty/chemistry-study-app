  ...
  return <div className="organic-knowledge-browser">
    ...
    <section className="organic-reaction-list" aria-live="polite"><h3>反応データ <small>{reactions.length}件</small></h3>
      {reactions.map(reaction=>{
        const body=<><div className="organic-reaction-route"><strong>{reaction.source.name}</strong><span>→</span><strong>{reaction.target.name}</strong></div><dl>{reaction.reagents.length>0&&<><dt>試薬</dt><dd>{reaction.reagents.join("・")}</dd></>}{reaction.catalysts.length>0&&<><dt>触媒</dt><dd>{reaction.catalysts.join("・")}</dd></>}{reaction.solvents.length>0&&<><dt>溶媒</dt><dd>{reaction.solvents.join("・")}</dd></>}{reaction.temperature&&<><dt>温度</dt><dd>{reaction.temperature}</dd></>}{reaction.pressure&&<><dt>圧力</dt><dd>{reaction.pressure}</dd></>}{reaction.otherConditions.length>0&&<><dt>その他条件</dt><dd>{reaction.otherConditions.join("・")}</dd></>}{reaction.byproducts.length>0&&<><dt>副生成物・現象</dt><dd>{reaction.byproducts.join("・")}</dd></>}{reaction.notes&&<><dt>注記</dt><dd>{reaction.notes}</dd></>}</dl></>;
