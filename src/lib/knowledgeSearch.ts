...
export type SearchItemType = "unit" | "ion" | "compound" | "reaction" | "precipitate" | "complex" | "gas" | "battery" | "industrial" | "basic";
...
function buildBaseIndex():SearchItem[]{
  ...
  const knowledge=chemistryUnits.filter(unit=>unit.slug!=="organic-reactions").flatMap(unit=>unit.sections.flatMap(section=>sectionItems(unit.slug,section)));
  ...
  return [...units,...compounds,...reactions,...gasItems,...batteries,...glossary,...formulas,...knowledge];
}
...
export const searchTypeLabels:Record<SearchItemType,string>={unit:"単元",ion:"イオン",compound:"有機化合物",reaction:"反応",precipitate:"沈殿",complex:"錯イオン",gas:"気体",battery:"電池・電気分解",industrial:"工業的反応",basic:"重要事項"};
